...
function equipPack()
	...
	welder.Part1 = player.HumanoidRootPart
...
function updateGUI()
    ...
    gui.Frame.BackgroundColor3 = Color3.new(255/timez, timez-255,0)
...
function thrust()
	if fuel > 0 then
		...
		bodyGyro.Parent = player.HumanoidRootPart
		while thrusting do
            ...
            if Tool.Handle.Thrusting.Playing == false then
			Tool.Handle.Thrusting:Play()
            end

			if fuel <= 200 then
				if Tool.Handle.LowFuelWarning.Playing == false then
