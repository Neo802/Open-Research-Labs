...
script.Parent.ClickDetector.MouseClick:connect(function(plr)
	...
	if activated == false then
		...
		if game.Workspace["MAIN.VALUES"].ALREADYEM.Value == false then
			...
			game.Workspace["MAIN.VALUES"].EMERGENCYCOOLANT.Value = true
			game.Workspace["MAIN.VALUES"].ALREADYEM.Value = true
			game.Workspace["MAIN.SOUNDS"].MELTDOWN.CountdownTheme:Stop()
			game.Workspace["MAIN.SOUNDS"].MELTDOWN.SoundBackground:Stop()
			game.Workspace["MAIN.SOUNDS"].MELTDOWN.ActivationSong:Play()
			game.Workspace["MAIN.VALUES"].ACTIVATOR.Value = plr.Name
			game.ReplicatedStorage.Badge:Fire(plr.Name,"ec")
			game.ReplicatedStorage.EmergencyCoolantACT:FireAllClients(plr.Name)
			---------------------------- END ----------------------------------
			wait(1) --DELAY	
...
game.ReplicatedStorage.CoolingProcess.OnServerEvent:connect(function()
...
game.ReplicatedStorage.EmergencyCoolantAPI.OnServerEvent:connect(function(plr,val)
	if act == false then -- TO PREVENT SPAM
		...
		if val == "true" then
			...
			game.Workspace["MAIN.SOUNDS"].MELTDOWN.ActivationSong:Stop()
			game.Workspace["MAIN.SOUNDS"].MELTDOWN.SuccessfullyOST:Play()
			for i=0x1,#meltdowndebris do
				meltdowndebris[i].Script.Disabled = true
			end
			wait(103)
			...
			--game.ServerScriptService.Nanobots.Disabled = false	
		--[[while true do
	        wait(.1)
		...
		elseif val == "false" then
			pcall(function()
				...
				if coolantplr ~= nil then
					game.ReplicatedStorage.Badge:Fire(coolantplr,"coolantplr")
				end
