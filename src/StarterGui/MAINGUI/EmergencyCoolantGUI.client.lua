game.ReplicatedStorage.EmergencyCoolantACT.OnClientEvent:connect(function(plr)
	if game.Workspace["MAIN.VALUES"].EMERGENCYCOOLANT.Value == true and script.Parent.VALUES.Intro.Value == false then
		...
		local TEXTONE = "The Emergency Coolblaster was activated by "..plr.."..."
		local TEXTTWO = plr.." was brave enough to activate the last solution to save the reactor core..."
		local TEXTTHREE = "Now let's watch the Emergency Coolblaster in action..."
		local TEXTFOUR = "The Emergency Coolblaster is being delivered to the reactor core..."
		--------------------------------------------------------------------------------------------------------
		local TEXTI = "Well, look at that! The coolant generator and the coolant control are enabled!"
		local TEXTII = "This means that the Emergency Coolblaster can reach to the reactor core with no issues!"
		local TEXTII5 = "Now let's see how the temperature is decreasing to normal."
		local TEXTIII = "Thanks to "..plr..", the meltdown has successfully been aborted!"
		...
		--------------------------------------------------------------------------------------------------------
		local TEXTIC = "Well, look at that! The coolant generator is enabled!"
		local TEXTIICN = "But since everyone was lazy enough to not enable the coolant control from the Control Room"
		local TEXTIIC = "But since "..coolantplr.." has disabled the coolant control from the Control Room"
		local TEXTIIC5 = "Emergency Coolblaster couldn't reach to the reactor core!"
		...
		--------------------------------------------------------------------------------------------------------
		local TEXTI2 = "O.o! The coolant geneator and the cooling control from the control room are disabled!"
		local TEXTII2 = "This means that the Emergency Coolblaster can't reach to the reactor core!"
		local TEXTIII2 = "The meltdown is not aborted, I give you one minute to evacuate the facility!"
		--------------------------------------------------------------------------------------------------------
		...
		if game.Workspace["MAIN.VALUES"].CUTSCENE.Value == 1 then
			...
			--[[if game.Players.LocalPlayer.Name == plr then
				game.ReplicatedStorage.Badge:FireServer(game.Players.LocalPlayer.Name,2124478582)
			end]]
			-------------------------------------
			wait(0.5)
		...
		if game.Workspace["MAIN.VALUES"].CUTSCENE.Value == 2 then
			...
			game.ReplicatedStorage.CoolingProcess:FireServer()
			wait(6)
		...
		if game.Workspace["MAIN.VALUES"].CUTSCENE.Value == 3 then
			...
			wait(1)
			if active.Value == false and ready.Value == false then	
				for i=1,#TEXTI2 do
				...
				game.ReplicatedStorage.EmergencyCoolantAPI:FireServer("false")
			...
			elseif active.Value == true and ready.Value == true then
				for i=1,#TEXTI do
				...
				if game.Workspace["MAIN.VALUES"].CUTSCENE.Value == 4 then
					...
					game.ReplicatedStorage.EmergencyCoolantAPI:FireServer("true")
					for i=1,25 do
						script.Parent.BlackScreen.BackgroundTransparency = script.Parent.BlackScreen.BackgroundTransparency + 0.1 
						wait()
					end
					...
					wait(2)
					GUI.Visible = false
					game.ReplicatedStorage.Money:FireServer(plr,10,"saving the core")
					game.StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.All, true)
					game:GetService("StarterGui"):SetCore("ResetButtonCallback", true)
